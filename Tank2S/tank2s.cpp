...
inline bool Symmetry(int x1, int y1, int x2, int y2)
{
    return (x1 == 8-x2 && y1 == 8-y2);
}
...
void InitTank()
{
    ...
    for (int i = 0; i < sz(destroyed_blocks); i += 2)
    {
        ...
        if (tankX[side][tank] >= 0)
        {
            ...
            if (ActionIsShoot(act))
            {
                ...
                while (true)
                {
                    ...
                    if (gameField[yy][xx] == Steel) break;
                }
            }
        }
    ...
    // 假设对方的坦克跟我们坦克行动完全一样， 根据地形的对称性猜对方的位置
    REP(tank, 2)
    if (tankAlive[mySide][tank] && !tankVisible(mySide, tank))
    {
        if (tankX[1-mySide][tank] >= 0) continue;
        int x = tankX[mySide][tank], y = tankY[mySide][tank];
        if (InAnotherSide(mySide, y)) continue;
        tankX[1-mySide][tank] = 8-x;
        tankY[1-mySide][tank] = 8-y;
    }

    REP(tank, 2)
    if (tankX[1-mySide][tank] == -2 && tankAlive[mySide][tank])
    {
        int sX = -2, sY = -2;
        for (int turn = 1; turn < currentTurn; turn++)
        {
            int x1 = ourLastX[turn][tank], y1 = ourLastY[turn][tank];
            int x2 = enemyLastX[turn][tank], y2 = enemyLastY[turn][tank];
            if (x2 == -2 && gameField[y1][x1] == Forest)
            {
                sX = 8-x1; sY = 8-y1;
                continue;
            }
            if (x2 != -2 && Symmetry(x1, y1, x2, y2)) continue;
            break;
        }

        tankX[1-mySide][tank] = sX; tankY[1-mySide][tank] = sY;
    }
    REP(tank, 2)
    {
        bool flag = true;
        int x, y;
        for (int turn = currentTurn-1; turn >= 0; turn--)
        {
            x = enemyLastX[turn][tank]; y = enemyLastY[turn][tank];
            if (x != -2) break;
        }
        if ((1-mySide == 0 && tankY[1-mySide][tank] < y) || (1-mySide == 1 && tankY[1-mySide][tank] > y))
            tankX[1-mySide][tank] = tankY[1-mySide][tank] = -2;
    }
    cerr << "Enemy position:\n";
    cerr << "0: " << tankY[1-mySide][0] << ' ' << tankX[1-mySide][0] << "\n";
    cerr << "1: " << tankY[1-mySide][1] << ' ' << tankX[1-mySide][1] << "\n";
}
...
void Greedy()
{
    ...
    {
        // if enemy tank and our tank in a same cell then stay one turn
